...
当前使用“每一级局部顺序编号”，编号只负责同级排序，不再把上级编号重复编码到文件名中：

```text
一级：10.数据技术
二级：03.数据应用
三级：03.OLAP
文章：02.事件分析-全流量聚合模型.md
```
...
- 不要复制其他文章的永久链接。当前 permalink 页面没有重复值。
...
`00.概览.md` 只用于确有导航和解释价值的主要知识目录，并设置 `article: false`。叶子目录名称已经足够清晰、或目录内只有一篇正文时，优先在上级概览中说明，不创建只有几条提纲的占位页。
