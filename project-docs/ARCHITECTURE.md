...
| `docs/00.目录页/` | 四个主栏目入口和三个旧数据入口兼容页；数据技术总入口为手写索引，其余目录页使用 `Catalogue` 组件 |
...
项目管理栏目按知识用途划分为四个同级模块：基础模块提供跨生命周期的原则与判断框架；预测型模块组织过程组、知识领域和ITTO；敏捷与混合型模块组织敏捷原则、团队方法与规模化实践；PMP认证备考模块按考试域、场景决策和练习复盘组织内容。文章移动时保留已有 permalink，目录结构负责知识导航，永久链接负责外部稳定访问。

首页栏目图存放在 `docs/.vuepress/public/img/home/`，统一使用 1200×800 JPG。图片由 AI 生成，图片本身不包含标题文字，栏目名称由页面 Frontmatter 渲染。
...
编号被直接当作数组下标，再通过 `filter` 压缩，因此能够排序。项目已改为每一级从 `00` 或 `01` 开始的局部顺序编号，避免使用六位全局编号形成大型稀疏数组。

目录页同时利用侧边栏数据生成目录树。`sidebar.catalogue` 保存一级栏目名称到目录页永久链接的映射，用于面包屑跳转。

`Catalogue.vue` 只能够安全渲染有限层级。数据技术合并后最深路径达到“数据技术 → 数据架构 → 数据平台 → 具体组件 → 文章”，超过组件支持范围，因此 `/data/` 使用普通 Markdown 索引连接 `/dm/`、`/data_structure/` 和 `/data_app/`。三个子目录页仍由 `Catalogue` 自动生成。
